...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let rootViewController = storyboard.instantiateInitialViewController() as? SplashViewController
        let navigationController = UINavigationController(rootViewController: rootViewController ?? UIViewController())

        rootViewController?.viewModel = DefaultSplashViewModel(
            router: DefaultRouter(navigationController: navigationController),
            userLoggedUseCase: DefaultUserLoggedUseCase(
